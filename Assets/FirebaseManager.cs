...
public class FirebaseManager : MonoBehaviour
{
    ...
    void FogotPassword(string forgotPasswordEmail)
    {
        auth.SendPasswordResetEmailAsync(forgotPasswordEmail).ContinueWithOnMainThread(RestoreTask => {
            ...
            confirmationPasswordText.text = "Si tu correo tiene una cuenta, el correo para reestablecer la contraseña ha sido enviado";
...
public class FirebaseManager : MonoBehaviour
{
    ...
    IEnumerator Login(string email, string password)
    {
        Debug.Log($"Starting login process with email: {email}");
        ...
        if (LoginTask.Exception != null)
        {
            ...
            if (firebaseEx != null)
            {
                Debug.LogWarning($"Firebase Error Code: {firebaseEx.ErrorCode}");
            }
            else
            {
                Debug.LogWarning("No se pudo obtener el error de Firebase.");
            }
            AuthError errorCode = (AuthError)firebaseEx.ErrorCode;
        ...
        else
        {
            ...
            if (user != null)
            {
                Debug.Log($"Usuario iniciado excitosamente: {user.DisplayName} ({user.Email})");
            }
            else
            {
                Debug.LogWarning("El usuario es nulo. No se puede acceder al DisplayName.");
            }
            warningLoginText.text = "";
            ...
            Debug.Log("Valor de user: " + user);
            Debug.Log("Valor de DisplayName: " + user.DisplayName);
